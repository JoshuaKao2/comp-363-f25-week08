...
while ticks < TOTAL_TICKS:
    # Moves cars that arrive at the destination node 
    for (src, dst) in list(in_transit.keys()):        
        pop_to_queue_if_arrived(src, dst)

    # processes cars at traffic light
    for node in nodes:
        serve_intersection(ticks, node)

    for src, dst in links:
        if is_boundary_incoming_link(src, dst):
            if random.random() < ARRIVAL_RATE:
                new_car = Car(id=car_id, t_enter=ticks)
                enqueue_departure(src, dst, new_car)
                car_id += 1

        #updates stats
        total_queue = sum(len(q) for q in stopped.values())
...
#report
print("Grid size: ", N, "x", N, ", Simulation Duration: ", TOTAL_TICKS)
print("Traffic lights cycle: NS green ", CYCLE_NS_GREEN, " EW green ", CYCLE_EW_GREEN)
print("Arrival rate: ", ARRIVAL_RATE, " cars/tick")
print("Total trips completed: ", completed)
print("Throughput: ", completed/TOTAL_TICKS)
print("Mean travel time: ", sum_tt/completed)
print("Mean num of queues vehicles: ", sum_queue/max(1, queue_samples))
